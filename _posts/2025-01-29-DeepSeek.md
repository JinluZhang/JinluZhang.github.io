...
在注意力（attention）机制上，DeepSeek-V3 采用 MLA 架构。记 $d$ 表示嵌入维度（embedding dimension），$n_h$ 表示注意力头的数量，$d_h$ 表示每个注意力头的维度，$\mathbf{h}_t \in \mathbb{R}^d$ 表示某一注意力层中第t个token对应的attention输入。 MLA的核心在于对key和value的低秩联合压缩low-rank joint compression，从而在推理阶段减少Key-Value cache：  

$$
\mathbf{c}_{t}^{KV} = W_{DKV} \mathbf{h}_t \tag{1}
$$

$$
[k_{t,1}^C; k_{t,2}^C; \dots; k_{t,nh}^C] = k_t^C = W_{UK} c_{t}^{KV} \tag{2}
$$

$$
k_t^R = \text{RoPE}(W_{KR} h_t) \tag{3}
$$

$$
k_{t,i} = [k_{t,i}^C; k_{t,i}^R] \tag{4}
$$

$$
[v_{t,1}^C; v_{t,2}^C; \dots; v_{t,nh}^C] = v_t^C = W_{UV} c_{t}^{KV} \tag{5}
$$

其中 $\mathbf{c}_{t}^{KV} \in \mathbb{R}^{d_c}$ 是keys和values经过压缩的latent向量，$d_c$ 为压缩维度；$W_{D\!K\!V} \in \mathbb{R}^{d_c \times d}$ 为向下投影矩阵（down-projection），\(W_{U\!K}, W_{U\!V} \in \mathbb{R}^{d_h n_h \times d_c}\) 分别是键和值的向上投影矩阵（up-projection）。\(W_{K\!R} \in \mathbb{R}^{d_R \times d}\) 用于生成带有旋转位置编码（RoPE, Su 等, 2024）的“解耦”键向量 \(k^R_t\)。\(\mathrm{RoPE}(\cdot)\) 表示将 RoPE 矩阵应用于输入向量，[·; ·] 表示向量拼接。需要注意的是，对于 MLA，在推理时只需缓存蓝色框所示的向量（即 \(c_{K\!V,t}\) 与 \(k^R_t\)），这大幅减少了 KV 缓存的大小，同时性能仍能与标准多头注意力（MHA, Vaswani 等, 2017）相当。
