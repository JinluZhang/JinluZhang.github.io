...
# 一些HighLight
 ...
 - deepseek一系列工作的创新点概括为：DeepSeekMoE、MLA（于V2提出）；FP8精度训练（于V3大规模验证）；MTP（于V3提出）；用R1数据作为强化学习的reasoning data，蒸馏了R1的推理能力（于V3提出）。这些创新点大都为了训练和推理的经济性，共同促成了deepseek如此惊艳的训练效率和推理效率，火出了圈。
