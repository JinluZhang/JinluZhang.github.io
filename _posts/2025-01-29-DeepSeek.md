...
$$
[\mathbf{q}_{t,1}; \mathbf{q}_{t,2}; \dots; \mathbf{q}_{t,n_h}] = \mathbf{q}_t, \tag{4}
$$

以第i个Attention头为例，记它第t个token对应的的k向量 $\mathbf{q}^i$ 如下，v类似。

$$
\mathbf{k}^i = [\mathbf{k}_{1,i}; \mathbf{k}_{2,i}; \dots; \mathbf{k}_{t,i}], \tag{5}
$$

$\mathbf{q}_{t,i}$ 要和所有keys计算MatMul，除以 $\sqrt{d_h}$ 后计算softmax，代表对所有values的权重。记 $\mathbf{q}_{t,i}$ 对应的输出是 $\mathbf{o}_{t,i}$ 

$$
\mathbf{o}_{t,i} = \text{Softmax}\bigl(\frac{q_{t,i} k^i}{\sqrt{d_h}}\bigr) v^i, \tag{6}
$$

然后o concat在一起后乘以$W^O \in \mathbb{R}^{d \times d_h n_h}$ ，就是第t个token对应的MHA的整体输出：

$$
\mathbf{u}_t = W^O [\mathbf{o}_{t,1}; \mathbf{q}_{t,2}; \dots; \mathbf{q}_{t,n_h}], \tag{7}
$$

在推理过程中，所有的 $\mathbf{k}, \mathbf{v}$ 需要缓存以加速推理，因此在推理时，需要缓存 2×𝑑×sequence_length 个元素。在模型部署时，这种庞大的 KV 缓存成为了限制maximum batch和sequence length的瓶颈。
...
其中，c_{KV} ∈ R^d_c 为压缩后的键值潜在向量；𝑑（≪ 𝑑_h）为 KV 压缩维度；𝑊_{DKV} ∈ R^d_c × d_h 为下投影矩阵；𝑊_{UK} 和 𝑊_{UV} ∈ R^d_h × d_c 分别为键和值的上投影矩阵。
在推理过程中，MLA 仅需要缓存 c_{KV}，因此其 KV 缓存只有 𝑑_c𝑙 个元素，其中 𝑙 表示层数。
此外，在推理时，由于 𝑊_{UK} 可以吸收到 𝑊𝑄 中，𝑊_{UV} 可以吸收到 𝑊_𝑂 中，因此我们甚至无需单独计算键和值来进行注意力操作。图 3 直观地展示了 MLA 中 KV 联合压缩如何减少 KV 缓存。

为了减少训练过程中的激活内存，我们还对查询进行了低秩压缩，尽管这不能减少 KV 缓存：

 

其中，c_{Q_t} ∈ R^{d_c'} 为压缩后的查询潜在向量；𝑑'（≪ 𝑑_h）为查询压缩维度；𝑊_{DQ} ∈ R^{d_c'} × d_h，𝑊_{UQ} ∈ R^{d_h} × d_c' 为查询的下投影和上投影矩阵。
