...
DeepSeek-V3 的整体框架仍然基于 Transformer（Vaswani 等, 2017）。为了实现高效推理与经济可控的训练成本，DeepSeek-V3 继续沿用 MLA 和 DeepSeekMoE，这两种方法在 DeepSeek-V2 中已得到充分验证。与 DeepSeek-V2 相比，主要的区别在于，我们在 DeepSeekMoE 中额外引入了一种**无辅助损失（auxiliary-loss-free）的负载均衡策略**（Wang 等, 2024a），以减轻因实现负载均衡而导致的性能下降。图 2 展示了 DeepSeek-V3 的基本架构，本节我们将简要回顾 MLA 与 DeepSeekMoE 的相关细节。

---

### 2.1.1 Multi-Head Latent Attention

在注意力（attention）机制上，DeepSeek-V3 采用 MLA 架构。设  
- \(d\) 表示嵌入维度（embedding dimension），  
- \(n\) 表示注意力头（attention heads）的数量，  
- \(d_h\) 表示每个注意力头的维度，  
- \(h_t \in \mathbb{R}^d\) 表示某一层注意力机制中第 \(t\) 个 token 的输入向量。  

MLA 的核心在于对注意力键（key）与值（value）的低秩联合压缩，从而在推理阶段减少键-值缓存（KV cache）。具体可表示为：  
\[
c_{K\!V,t} = W_{D\!K\!V} \, h_t,
\tag{1}
\]  
\[
[k^C_{t,1}; k^C_{t,2}; \dots; k^C_{t,n_h}]
=
k^C_t
=
W_{U\!K} \, c_{K\!V,t},
...
\]  
\[
k^R_t = \mathrm{RoPE}\bigl(W_{K\!R} \, h_t\bigr),
\tag{3}
\]  
\[
...
\]  
\[
[v^C_{t,1}; v^C_{t,2}; \dots; v^C_{t,n_h}]
=
v^C_t
=
W_{U\!V} \, c_{K\!V,t},
\tag{5}
\]  
其中 \(c_{K\!V,t} \in \mathbb{R}^{d_c}\) 是键值的压缩潜在向量，\(d_c (\ll d_h \cdot n_h)\) 为键-值的压缩维度；\(W_{D\!K\!V} \in \mathbb{R}^{d_c \times d}\) 为向下投影矩阵（down-projection），\(W_{U\!K}, W_{U\!V} \in \mathbb{R}^{d_h n_h \times d_c}\) 分别是键和值的向上投影矩阵（up-projection）。\(W_{K\!R} \in \mathbb{R}^{d_R \times d}\) 用于生成带有旋转位置编码（RoPE, Su 等, 2024）的“解耦”键向量 \(k^R_t\)。\(\mathrm{RoPE}(\cdot)\) 表示将 RoPE 矩阵应用于输入向量，[·; ·] 表示向量拼接。需要注意的是，对于 MLA，在推理时只需缓存蓝色框所示的向量（即 \(c_{K\!V,t}\) 与 \(k^R_t\)），这大幅减少了 KV 缓存的大小，同时性能仍能与标准多头注意力（MHA, Vaswani 等, 2017）相当。
...
\]  
\[
[q^C_{t,1}; q^C_{t,2}; \dots; q^C_{t,n_h}]
=
q^C_t
=
W_{U\!Q} \, c_{Q,t},
