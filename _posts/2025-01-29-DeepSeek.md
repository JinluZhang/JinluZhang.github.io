...
为了解决这个问题，我们提出了一种解耦 RoPE 策略，该策略使用额外的多头queries $\mathbf{q}_{t,i}^R \in \mathbb{R}^{d_h^R}$ 和一个共享key $\mathbf{k}_{t}^R \in \mathbb{R}^{d_h^R}$ 来携带RoPE信息，其中 $d_h^R$ 代表每个头中，解耦了的queries和key的维度大小。通过 RoPE 解耦策略，MLA 执行以下计算：
